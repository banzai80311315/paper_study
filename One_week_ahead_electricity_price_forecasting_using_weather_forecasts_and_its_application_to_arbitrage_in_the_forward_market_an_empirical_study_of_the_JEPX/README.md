...
元データは全研究共通の `paper_study/data/raw/` にあります。加工結果・特徴量・初期値は、この研究の `One_week_ahead_electricity_price_forecasting_using_weather_forecasts_and_its_application_to_arbitrage_in_the_forward_market_an_empirical_study_of_the_JEPX/data/` に保存します。Notebookでは最初のコードセルで設定する `RAW_DATA_DIR` と `PROJECT_DATA_DIR` を使用します。
